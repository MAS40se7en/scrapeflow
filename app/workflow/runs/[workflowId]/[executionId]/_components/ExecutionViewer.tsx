...
export default function ExecutionViewer({initialData}: {initialData: ExecutionData}) {
  ...
  const query = useQuery({
    ...
    refreshInterval: (q) => q.state.data?.status === ExecutionStatus.RUNNING ? 1000 : false,
  });
    ...
    <div className='flex w-ful h-full'>
      <aside className='w-[440px] min-w-[440px] max-w-[440px] border-r-2 border-separate flex flex-grow flex-col overflow-hidden '>
        <div className='py-4 px-2'>
          ...
          <ExecutionLabel icon={CalendarIcon} label="Started at" value={<span className='lowercase'>
        ...
        <div className='flex justify-center items-center py-2 px-4'>
          <div className='text-muted-foreground flex items-cemter gap-2'>
